...
```{r dropjune15}
## Using the individual level data created from the aggregated files (so, without ability to identify anyone).
wrkdat3 <- wrkdat
wrkdat3 <- wrkdat3 %>%
...
Simple description of total vaccinations and total assignments to arm ---
ignoring day-by-day design. Just to simply learn about how many vaccinations
occurred.
...
plot1 <-
    ggarrange(g1+theme(text=element_text(size=14),axis.text.x=element_text(size=10)),
    g2+theme(text=element_text(size=14),axis.text.x=element_text(size=8)),
    g3+theme(text=element_text(size=14),axis.text.x=element_text(size=10)), 
    nrow = 3, labels = c("Iteration 1", "Iteration 2", "Iteration 3"))
# print(plot1)
...
ggsave(file = "rq2_rq7_plot.pdf", path = OUTPUT_DIR, plot = rq2_rq7_plot, device = cairo_pdf, width = 13, height = 6)
...
```{r eq1, eval=FALSE}
## Don't run because we don't have ZCTA merged onto the aggregated data.
wrkdat3_eq1 <- wrkdat3 %>%
